...
#define kuan 360
#define chang 510
bool timer(int ms,int id) {//!!!  在循环中时 q在while下反复取值当对时才返回1 其余都是0
	int q = clock();
	if (q - timee[id] >= ms)
	{
		timee[id] = q;
...
void bgmf() {
	mciSendString("open ./资源/BGM04.mp3 alias bgm1", 0, 0, 0);//alias+name为取小名 0，0，0是音乐参数
...
bool draw() {
	...
	sprintf(ch, "%d", p.score);//用这个转换类型才行
	...
	if (snacksum < 3) {
		while (1) {//不刷蛇身上
			s1.x = rand()%chang + 1-5;
			s1.y = rand()%kuan + 1-5;
			for (int i = 0; i < p.size; i++) {//这次利用重心解决碰撞问题
	...
	//自身碰撞
	for (int i = p.size; i >=3; i--) {//不可能撞1，2
...
int main() {
	...
	initgraph(chang, kuan, SHOWCONSOLE);//debug用这个！！！！！且把代码放到窗口代码的下面
	
	
	init();
	while (true) {
		setbkcolor(RGB(28, 140, 119));//红绿黄
		cleardevice();//要加这个上面的才有用
